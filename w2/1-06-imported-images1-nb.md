...
```{code-cell} ipython3
# votre code
print(img2.flags.writeable)
img3 = np.copy(img2)
print(img3.flags.writeable)
```
...
```{code-cell} ipython3
# votre code
print(type(img3), img3.dtype, img3.shape, sep='\n')
```
...
```{code-cell} ipython3
# votre code
plt.imshow(img4[::2,::2,:]); plt.show()
plt.imshow(img4[::5,::5,:]); plt.show()
plt.imshow(img4[::10,::10,:]); plt.show()
plt.imshow(img4[::20,::20,:])
```
...
H, L, _ = img4.shape
plt.imshow(img4[(H-l)//2:(H+l)//2 , (L-c)//2:(L+c)//2 , :])
plt.show()
...
# L'affichage ne sait pas que le tableau correspond à l'intensité en rouge (ou vert, ou bleu) d'une image. 
# Il affiche simplement une image mettant en évidence les différences entre les valeurs du tableau
```
...
# votre code
img6 = np.copy(img5)
img6[-200:, -200:] = np.array((219,112,147))
plt.imshow(img6)
plt.show()
...
```{code-cell} ipython3
# votre code
img7 = plt.imread("data/les-mines.jpg")
```
...
# votre code
h,l,_ = np.shape(img7)
imga = np.zeros((h,l,4), dtype=np.uint8)
```
...
```{code-cell} ipython3
# votre code
img8 = plt.imread("data/les-mines.jpg")
```
...
```{code-cell} ipython3
# votre code
A = (imgf[:,:,0] + imgf[:,:,1] + imgf[:,:,2])/3
B = 0.399*imgf[:,:,0] + 0.587*imgf[:,:,1] + 0.114*imgf[:,:,2]

plt.imshow(A, cmap="Grays"); plt.show()
...
```{code-cell} ipython3
# votre code
Aint = (A*255).astype(np.uint8)
plt.imshow(Aint, cmap="Grays")
```
...
axes[0].imshow(A,cmap="Grays")
axes[1].imshow(B,cmap="Grays")
axes[2].imshow(A2,cmap="Grays")
```
...
axes[0, 0].imshow(A,cmap="Grays")
axes[0, 1].imshow(B,cmap="Grays")
axes[0, 2].imshow(A2,cmap="Grays")

axes[1, 0].imshow(A2,cmap="Grays")
axes[1, 1].imshow(A,cmap="Grays")
axes[1, 2].imshow(B,cmap="Grays")

axes[2, 0].imshow(B,cmap="Grays")
axes[2, 1].imshow(A2,cmap="Grays")
axes[2, 2].imshow(A,cmap="Grays")
```
