...
def rectangle_size(n):
    ...
    while x>rn//3: # choix arbitraire, une ligne collerait parfaitement
...
def patchwork(colors, side=10, background=[169, 169, 169]):
    ...
    I, J = np.indices((l*side,L*side))
    indices = (J + L*side*(I//side))//side
    img = c[np.minimum(indices, n-1)] 
    img[indices>=n] = bg
...
```{code-cell} ipython3
# votre code
def read_couleurs(file_name):
    d = {}
    with open(file_name, 'r') as f:
        for line in f:
            nom, r, g, b = line.split()
            d[nom] = list(map(int, (r,g,b)))
    return d
```
...
```{code-cell} ipython3
# votre code
d = read_couleurs("data/rgb-codes.txt")
print(d['Red'])
print(d['Lime'])
print(d['Blue'])
```
...
def patchwork2(color_names, side=10, background_color="DarkGray"):
    ...
    d = read_couleurs("data/rgb-codes.txt")
    c_n = np.array([d[name] for name in color_names])
    n = len(color_names)
    l, L = rectangle_size(n)
    print(l,L)
    ...
    I, J = np.indices((l*side,L*side))
    indices = (J + L*side*(I//side))//side
    img_nom = c_n[np.minimum(indices, n-1)]
    img_nom[indices>=n] = bg

    #print(indices)
    return img_nom.astype(np.uint8)
```
...
d = read_couleurs("data/rgb-codes.txt")
noms = list(d.keys())
color_list = [noms[randint(0, len(noms)-1)] for _ in range(nb)]
plt.imshow(patchwork2(color_list));
```
...
# votre code
d = read_couleurs("data/rgb-codes.txt")
noms = np.array(list(d.keys()))
filtre = np.vectorize(lambda nom: "WHITE" in nom.upper())(noms)

color_list = noms[filtre]
print(color_list)
plt.imshow(patchwork2(color_list));
```
...
d = read_couleurs("data/rgb-codes.txt")
noms = list(d.keys())
plt.imsave("patchwork.png", patchwork2(noms));
```
...
def sepia(img):
    n, m, _ = img.shape
    coef = np.array([0.393, 0.769, 0.189, 0.349, 0.686, 0.168, 0.272, 0.534, 0.131]).reshape((3,3)).T

    img1 = img.astype(np.float64)
    img2 = img1.dot(coef)
    img3 = np.minimum(img2, 255.)
